        ...
        try {
            ...
            PreparedStatement preparedStatement =
                    connection.prepareStatement
                            ("SELECT * FROM homework_day1_mysql.offers " +
                            "WHERE promoted = 0 AND price > 50000 AND title LIKE '%l' ");
            ...
            PreparedStatement preparedStatement1 = connection.prepareStatement
                    ("SELECT COUNT(homework_day1_mysql.offers.id) AS sum_offers " +
                            "FROM homework_day1_mysql.offers " +
                            "WHERE status = 1 AND promoted_to > CURRENT_DATE and expire < NOW() and price > 300000");
            ...
            PreparedStatement preparedStatement4 = connection.prepareStatement
                    ("SELECT homework_day1_mysql.offers.title AS offersTitle " +
                            "FROM homework_day1_mysql.offers ORDER BY id DESC");
            ...
            PreparedStatement preparedStatement9 = connection.prepareStatement
                    ("SELECT ROUND(AVG(price), 2) AS avg_price " +
                            "FROM homework_day1_mysql.offers " +
                            "WHERE expire > NOW() AND promoted = 1 AND price > 44645.04");
            printResults(preparedStatement9, "avg_price");
